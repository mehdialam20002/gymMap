...
| `rel.iam.password-authentication` | Release | **Off:** the four `/v1/auth/*` password routes return `404` and phone OTP is the only way in — which is the `FR-AUTH-01` consumer default anyway, so a member registering on the website is unaffected. Turning it off does NOT touch the lockout policy, the Argon2id parameters, the audit trail or any existing session: a password already set stays set and stays hashed, and a member already signed in stays signed in. It removes a way IN, never a protection. | OFF | percentage | Product | 2026-08-07 | 2026-11-05 | `FR-AUTH-04`, `FR-AUTH-08`, `FR-AUTH-10`, `AC-EP02-07` | PROPOSED |
| `rel.iam.phone-otp-login` | Release | **Off:** the two `/v1/auth/otp/*` routes return `404` and email-plus-password is the only way in. Redis keys expire in 300 seconds, so a rollback leaves no residue and no member is stranded mid-flow for longer than five minutes. Turning it off does NOT relax any limit: the per-number budget, the per-IP ceiling, the captcha threshold and the five-attempt cap are all inert rather than loosened, because the routes that consult them do not answer. | OFF | percentage | Product | 2026-08-08 | 2026-11-06 | `FR-AUTH-05`, `AC-AUTH-01.3`, `AC-AUTH-01.4`, `AC-AUTH-01.5`, `CON-02` | PROPOSED |
| `rel.iam.session-rotation` | Release | **Off:** a refresh token is issued at login and accepted on refresh, but is NOT rotated on use — the same generation keeps working for its thirty days. A strictly weaker posture that keeps every member signed in while a rotation defect is diagnosed, which is the point: the alternative rollback is signing out the entire platform. It does **NOT** disable reuse detection or family revocation, and it cannot: `FEATURE_FLAGS.md` §2.2 forbids flagging a security control off, and with rotation paused there is no spent generation for a replay to present. The `TR-28` grace window, the AC-10 family denylist and `POST /v1/auth/logout` are all unaffected. | ON | percentage | Engineering | 2026-08-08 | 2026-11-06 | `FR-AUTH-06`, `FR-AUTH-09`, `E1.2`, `TR-28`, ADR-0011 | PROPOSED |
