...
| **A-42** | Malware scanning for uploads | **ClamAV** (`clamav/clamav`) as a sidecar daemon in the worker tier, spoken to over its INSTREAM socket by the upload pipeline (`Security.md` §9.3) | `NFR-SEC-10` requires every upload to be *"virus-scanned"* and names no engine; `PROJECT_CONSTITUTION.md` §12.7 `UP3` requires the scan to happen **before the object becomes retrievable**. `Security.md` §0.4 already carried this as the recommended engine at `PROPOSED` and had already rejected both alternatives with reasons: a **cloud provider scanning service** adds a sub-processor under `NFR-PRV-06` and a residency question under `NFR-PRV-05` — the files are gym owners' PAN cards, passports and bank proofs, and `OQ-16` keeps them in India; a **commercial engine** costs against `CON-05`. ClamAV is free, self-hosted, and no byte leaves the estate. Approved by the project owner on 2026-08-10 — see `ADR-0048`. **Numbered outside the contested block on purpose**, for the same reason `A-40` was | `APPROVED` |
