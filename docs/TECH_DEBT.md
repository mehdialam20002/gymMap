...
| **TD-045** | **HALF PAID 2026-08-10. `PermissionsGuard` IS NOW BOUND.** Four guards were built, unit-tested and registered NOWHERE — `PermissionsGuard`, `MfaGuard`, `ResourceTenantGuard`, `ImpersonationRestrictionGuard`.** `grep -rn "APP_GUARD" apps/server/src` → four hits, all in `app.module.ts`, binding only `JwtAuthGuard`, `TenantGuard`, `PlatformRoleGuard`. `grep -rn "UseGuards" apps/server/src` → **zero**. So `FR-RBAC-01` and `FR-RBAC-02` are unenforced at runtime across the whole API, and `NFR-SEC-11`'s mandatory staff second factor never runs | Security | **High** | M | Cannot be taken as one step — see the entry. Global registration today **403s every authenticated route**, because all seven declared permission keys sit outside `PERMISSION_KEYS`. The order is: resolve the keys, then register | Backend Lead + Security | **OPEN** | `FR-RBAC-01`, `FR-RBAC-02`, `NFR-SEC-11` · `Security.md` §916 guard order · `BLK-10`, `BLK-14`, `BLK-19` · M-023, M-024, M-025 **What was paid:** all seven declared route keys were resolved first — two became `§B3.2` rows 44/45 (`ADR-0047`), three were WRONG STRINGS the catalogue already froze (`iam.session.list`, `iam.mfa.enrol/verify/disable` — and the MFA case had collapsed three catalogued keys into one), and the rest got two explicit admit lists: `SELF_SERVICE_PERMISSIONS` (caller's own row, handler scopes it) and `SCOPED_NON_MATRIX_PERMISSIONS` (tenant data, so `scopeReaches()` still decides — admitting `tenancy.ping.read` the self-service way would let any principal ping any tenant, which is `BR-TEN-01`). Enumerated strings, never prefixes: `startsWith('iam.session.')` is a rule an attacker satisfies by naming a route well. `PermissionsGuard` is bound after `JwtAuthGuard` per `Security.md`'s fixed pipeline, and `permissions-guard-registered.spec.ts` reads `app.module.ts` textually — a guard's own unit tests cannot notice that nothing calls it, which is how this survived. **What remains: `MfaGuard`, `ResourceTenantGuard`, `ImpersonationRestrictionGuard`.** `MfaGuard` is blocked on the seed, not on keys: all eleven seeded principals have `password_hash` NULL and enrolment re-authenticates against that hash, so binding the mandate today locks every platform account out of every route with no path back. A test now asserts its ABSENCE so binding it is deliberate. **THREE OF FOUR PAID as of 2026-08-11** (`d4da199`): `ImpersonationRestrictionGuard` is now registered globally, after `PermissionsGuard` — the order matters, because an agent who lacks the permission anyway should be told that rather than being told impersonation forbids it, and reversed the impersonation refusal would mask every ordinary authorisation failure on a financial route. **Two remain, and they are not the same kind of remaining:** `MfaGuard` is blocked on `TD-041` — the eleven principals' password hashes reach a FRESH database and not this one, so binding the mandate still locks every platform account out; `ResourceTenantGuard` is not blocked by anything and is simply unscheduled, which is worth saying plainly rather than letting it sit in a list beside a genuinely blocked one. |
| **TD-046** | **No runtime rate limiting exists.** `@RateLimit()` is `SetMetadata` + `ApiExtension` only; nothing reads `RATE_LIMIT_CLASS`. All 26 declarations across five controllers — every `/v1/auth/*` login, register, password-reset and OTP route — enforce nothing. `rate-limiter-flexible` is installed and never imported | Security | **High** | M | Before any public exposure. **Blocked on a prerequisite:** there is no trust-proxy configuration anywhere, so every per-IP bucket would be ONE bucket for all traffic — `RL-AUTH` 60/hour would mean 60 logins per hour **platform-wide** | Backend Lead + Security | **OPEN** | `API_Catalog.md` §4.2 nine classes, `RLM2`, `RL-3`, `RL-5` · `NFR-SEC-06` |
| **TD-048** | **`elevation_scope` has NINE documented values and FOUR coded ones, with ZERO overlap.** `Admin.md` writes `KYC_DOCUMENT_ACCESS`, `PLATFORM_SETTLEMENT_REVIEW`, `PLATFORM_REFUND_QUEUE`, `PLATFORM_DISPUTE_QUEUE`, `DISPUTE_EVIDENCE_ACCESS`, `AUDIT_SEARCH`, `PLATFORM_ANALYTICS`, `PLATFORM_MODERATION_QUEUE`, `PLATFORM_RECONCILIATION_READ`. `ELEVATION_SCOPES` in `tenancy/prisma/platform-elevation.ts` holds `READ_ALL_TENANTS`, `READ_ONE_TENANT`, `READ_FINANCIAL_AGGREGATE`, `READ_AUDIT` — **not one of which appears in any document**. The code invented the vocabulary, which is the fifth occurrence of that exact shape found on 2026-08-10 (after `catalog.branch.write`, `support.impersonation.create`, `iam.impersonation.manage` and the five `iam.own_*` keys) | Correctness | **High** | M | Immediately — it is not latent. `FR-ADMN-09` asks *"who looked at this owner’s passport, when, and why"* and the audit explorer answers it by filtering `elevation_scope`. With the code writing `READ_ONE_TENANT` where `Admin.md` says `KYC_DOCUMENT_ACCESS`, that query returns **nothing**, and a blank result reads as "nobody looked" | Backend Lead + Security | **OPEN** | `Admin.md` §3.4, §5.2 · `Schema.md` §13.3 · `PE2` · `FR-ADMN-09` · `BR-DAT-07` · `BLK-17` · M-013, M-023 |
| **TD-047** | **Impersonation is inert at runtime.** `AccessTokenVerifier.verify()` refuses `typ !== 'ACCESS'`, so the `IMPERSONATION` token is rejected by the global `JwtAuthGuard` on every route — including `POST /auth/impersonate/end`, which `Authentication.md` §8.15 requires be called **with** that token. Meanwhile `POST /auth/impersonate` still succeeds and writes an `IMPERSONATE_START` audit row, so the audit log records sessions that never happened | Correctness | **High** | M | Not a one-line relaxation. Lifting the `typ` check without registering `PermissionsGuard` in the same change grants the session the subject's **full** permission set — the union-by-omission `AC-5` forbids. Ordered behind `TD-045` | Backend Lead + Security | ✅ **REPAID 2026-08-11** (`d4da199`). **The ordering this row insisted on was followed exactly, and it was the whole of the debt.** `PermissionsGuard` was bound first (`c9c851e`), so `effectiveGrants()` — which intersects the agent's grants with the subject's and returns NOTHING when the agent's are absent — was already standing between an accepted token and a permission decision. Only then was `typ: 'IMPERSONATION'` accepted. **Four things were connected, each of which had passing unit tests and no caller:** `runAsImpersonator` gained a production caller (`ImpersonationContextMiddleware`), so `audit_log.impersonated_by` is no longer `null` on every row ever written — `AC-7` asks for the agent on EVERY write and it was on none; `impersonationExpired()` is enforced in `JwtAuthGuard`, the one place that rejects, against `imp_at` rather than `exp`; `mayElevate()` is consulted in `runElevated()`; and `ImpersonationRestrictionGuard` is registered globally, **after** `PermissionsGuard` so an agent who lacks the permission anyway is told that rather than being told impersonation forbids it. **Three structural checks added at the verifier**, because each missing claim fails OPEN downstream and silently. **And one correction worth keeping:** `runElevated()`'s impersonation guard was `actor.permission.startsWith('impersonation.')` — a string prefix on a caller-supplied field, which its own comment called *"the scaffolded principal"*. An elevation under a borrowed identity citing `admin.tenant.read` passed it cleanly. It now reads `currentImpersonation()`, which is a fact about the session. `impersonation-wiring.spec.ts` — 17 assertions on the CONNECTIONS, which is the part a unit test structurally cannot see. **Still owed at `M-025`, and not this debt:** `docs/features/impersonation.md`, `impersonation.contract-spec.ts` and two integration specs. | `FR-AUTH-12` · `Security.md` §2.9 `IM-1`…`IM-14`, §916 · `Authentication.md` §8.14/§8.15 · `BR-DAT-01` · `TD-045` · M-025 |
| **TD-041** | The dev database holds reference rows under the pre-ADR-0038 seed namespace; a reset and re-seed is owed | Data | Medium | S | Any run of `roles-seed.int-spec.ts`, which fails today and is a true positive | Backend | **OPEN** | `SeedStrategy.md` DT2 · §7.2 · M-031 **SHARPENED 2026-08-10, and it is worse than a stale namespace.** Measured against the live dev database today: `permissions` holds **64** rows where the code derives **77**, `role_permissions` holds 191, and **2 of 43 users have a password_hash** — none of them among the eleven §6.6 principals. Twenty isolation assertions fail on this, every one a true positive about the DATABASE rather than the code. **The part that does not fix itself:** `users.ts` emits `ON CONFLICT (id) DO NOTHING`, so `d6ad061`'s password hashes reach a FRESH database and never an existing one — the eleven rows are already there, and `DO NOTHING` skips them. `SEED_VERSION 0.5` flags the divergence and cannot repair it. So `MfaGuard` remains unbindable HERE even though the seed that blocked it is fixed, and re-running the seeder changes nothing. What is owed is either a reset — which destroys the owner's demo data and is theirs to authorise — or an idempotent `DO UPDATE` path for the columns a re-seed must be able to correct. `TD-042` compounds it: `pnpm db:seed` points at a path that does not resolve, so there is no supported way to re-run it at all. **SHARPENED AGAIN 2026-08-11, and it now blocks a whole acceptance criterion.** `M-031`'s five branch routes gained their isolation case groups — `ci:isolation-coverage` reports **7/7 covered, 0 uncovered**, which is `AC-8` — and the groups then fail **23 of 31** in `tenancy.isolation-spec.ts` with `403 !== 200` on `A4`, the POSITIVE control. The cause is measured, not inferred: `SELECT EXISTS(SELECT 1 FROM permissions WHERE key='catalog.branch.list')` returns **false**, `permissions` holds **64** rows where the code now derives **83**, and `GYM_OWNER` has 34 `role_permissions`. Every key `ADR-0047` added is absent, so `PermissionsGuard` correctly denies a route the code correctly grants. **The pre-existing `/v1/tenant/{tenantRef}/ping` cases fail identically**, which is how the cause was separated from the new code. Two consequences worth stating plainly: `AC-8` is *covered* and *not passing*, and the gap is one re-seed wide; and the suite is now LOUD about it where the same routes previously had no case group at all and were silently indistinguishable from passing. **`pnpm db:seed` itself was repaired on the same day** — `describePermission()` threw on every `extraReadKeys` entry, so the seeder could not generate SQL at all — which removes one of the two obstacles. What remains is exactly what this row has always said: a reset that destroys the owner's demo data and is theirs to authorise, or an idempotent `DO UPDATE` path for the columns a re-seed must be able to correct. |
...
| **TD-050** | **Two `branches` indexes are spelled differently in the database than in `Indexes.md`.** Specified `uq_branches__gym_id__primary`, shipped `uq_branches__one_primary_per_gym`; specified `idx_branches__city_id_status`, shipped `idx_branches__city_status`. Both have the correct columns, predicate and uniqueness — only the label differs, so nothing behaves wrongly and no query is affected. Found 2026-08-11 while reconciling `Indexes.md` §11 against the live catalogue, which is the same pass that found **three specified indexes missing entirely** (those were built, `20260811000000`) | Data | Low | S | **Any migration that already has to drop and recreate either index** — a partial-predicate change, a column addition, or the `CONCURRENTLY` rebuild `MG4` requires once `branches` is populated in a deployed environment. Not before: renaming is a `DROP`/`CREATE` pair on a live constraint for zero behavioural gain | Schema Owner | **ACCEPTED** | `Indexes.md` §11 rows 401, 403 · `NamingConvention.md` · `20260810120000_expand_create_branches` · `20260811000000_expand_add_missing_branch_indexes` · `docs/database/catalog.md` §1 · M-031 |
| **TD-049** | **`react-router-dom` has no approval row, and the gate that exists to catch that reports OK.** `apps/admin-dashboard/package.json:31` declares `"react-router-dom": "^6.28.1"`. `STACK_ADDITIONS.md` has **no `A-NN` row for client-side routing** — the rows run A-01…A-30 plus A-40, A-41, A-42, the only match for "router" in the document is Next.js's *App Router* (line 43, a different app), and Part 1's own entry is `Dashboards (both) \| React 18 + Vite + TypeScript (SPA)`, which names no router. **The interesting half:** `pnpm ci:deps-approved` passes — *"every dependency maps to an approved A-NN row"* — because `dependency-approval.mjs:139` lists `react-router-dom` in its allowlist under the comment **`// Framework runtimes named in Part 1`**. Part 1 does not name it. The gate is green on a provenance the register never granted, which is a **seventh** instance of the shape first recorded at `TD-048`: rank-5 code asserts something a rank-3 document was supposed to fix, and nothing fails because both sides are internally consistent. Found 2026-08-11 while triaging Dependabot's `Bump react-router-dom from 6.30.4 to 7.18.2` — the majors rule correctly refuses the bump, but refusing a bump silently assumes the 6.x underneath it was approved | Architecture | **High** | S | **Immediately, and in two parts.** The comment is the urgent half: an allowlist entry that cites a source which does not say what it claims is worse than no gate, because it converts an unreviewed dependency into a *documented* one and every future reader stops there. Fix that in the next change to the file even if the row takes longer. The row itself is owner work — the two React 18 + Vite dashboards need *a* router, the slot is genuinely open, and v6 is the ordinary choice, so the expected resolution is a new `A-NN` the owner approves rather than a removal. Note also that the gate is one-directional **by design** (its own header: *"a prefix here with no row is NOT checked, because the scope entries have no row by construction"*), so no amount of re-running finds the next one of these | Project owner + Technical Lead | **OPEN** | `STACK_ADDITIONS.md` Part 1 · CLAUDE.md §5 standing rule · `dependency-approval.mjs:130-139` · `admin-dashboard/package.json:31` · `TD-048` · `.github/dependabot.yml` |
| **TD-051** | **Twenty unnamed type and duration values in `customer-web`'s identity layer.** `ADR-0052` scoped `TS4`'s eleven-step type scale out of this surface so the "Chalk & Iron" identity could keep its voice, and it was explicit that this is **debt, not licence**. What is unnamed today: **13 off-scale fixed sizes** (9.5 / 10.5 / 11.5 / 12.5 / 13.5 / 14.5 / 15 / 19 / 22 / 26 / 30 px, now expressed in `rem`) and **7 fluid `clamp()` ramps**, plus **6 raw animation durations** above the `deliberate` 400 ms ceiling (`gm-rise 1.05s`, `gm-slide 34s`, a `transform 1.1s`, `gm-scan 2.6s`, `gm-orbit-float 5s`, `gm-orbit-pulse 2.8s`) and two raw `ease-in-out` keywords. Every one of them is a considered value that the token scale has no room for — a 34-second marquee cannot be a 400 ms transition — so the fix is to NAME them in the identity block, not to shorten them. Found 2026-08-11 by the responsive audit; the sizes were converted px → rem in the same pass **without changing any value**, precisely so this can land as one rename rather than a second sweep | Code | Low | M | **The next change that touches the type scale or the motion tokens for this surface** — a second identity, a dark/light divergence in type, or the day a designer asks for one more step. Also immediately if a `customer-web` value is ever copied into `packages/ui`, because that is the moment an unnamed number becomes a shared one. Not before: the values are correct, they are all in one file, and renaming 20 declarations for hygiene alone buys nothing a reader can see | Frontend Lead, customer site | **ACCEPTED** | `ADR-0052` · `DesignSystem.md` §5.3 `TS4` · §6.6 motion · `TK4` · `globals.css` identity block · `packages/ui/src/tokens/tokens.css` |
