...
| **TD-045** | **Four guards are built, unit-tested and registered NOWHERE — `PermissionsGuard`, `MfaGuard`, `ResourceTenantGuard`, `ImpersonationRestrictionGuard`.** `grep -rn "APP_GUARD" apps/server/src` → four hits, all in `app.module.ts`, binding only `JwtAuthGuard`, `TenantGuard`, `PlatformRoleGuard`. `grep -rn "UseGuards" apps/server/src` → **zero**. So `FR-RBAC-01` and `FR-RBAC-02` are unenforced at runtime across the whole API, and `NFR-SEC-11`'s mandatory staff second factor never runs | Security | **High** | M | Cannot be taken as one step — see the entry. Global registration today **403s every authenticated route**, because all seven declared permission keys sit outside `PERMISSION_KEYS`. The order is: resolve the keys, then register | Backend Lead + Security | **OPEN** | `FR-RBAC-01`, `FR-RBAC-02`, `NFR-SEC-11` · `Security.md` §916 guard order · `BLK-10`, `BLK-14`, `BLK-19` · M-023, M-024, M-025 |
