...
| **TD-049** | **`react-router-dom` is a dependency with no approval row.** `apps/admin-dashboard/package.json:31` declares `"react-router-dom": "^6.28.1"`. `STACK_ADDITIONS.md` has **no `A-NN` row for client-side routing** — the thirty-three rows run A-01…A-30 plus A-40, A-41, A-42, and the only match for "router" in the whole document is Next.js's *App Router* on line 43, which is a different app. Per the CLAUDE.md §5 standing rule that makes it *"a review blocker"*, not a debt to schedule. Found on 2026-08-11 while triaging why Dependabot had opened `Bump react-router-dom from 6.30.4 to 7.18.2` — the bump is correctly refused by the new majors rule, but the refusal assumes the 6.x underneath it was approved, and it was not | Architecture | Medium | S | **Before `admin-dashboard` builds its first route.** Not urgent as risk — v6 is the ordinary choice and nothing is wrong with it — but urgent as governance, because the longer it ships unlisted the more the answer becomes "it is already there", which is precisely the reasoning `STACK_ADDITIONS.md` Part 1 exists to refuse. The two React 18 + Vite dashboards need *a* router and the slot is genuinely open, so the expected resolution is a new `A-NN` row the owner approves, not a removal | Project owner + Technical Lead | **OPEN** | `STACK_ADDITIONS.md` Part 1 standing rule · CLAUDE.md §5 · `admin-dashboard/package.json:31` · `.github/dependabot.yml` |
