...
| :--- | :--- |
| Can anyone authenticate? | **Yes, with a real session, as of M-022.** `POST /v1/auth/login` and `POST /v1/auth/otp/verify` each return a 15-minute ACCESS token in the body and set the 30-day `__Host-gm_rt` REFRESH cookie — httpOnly, Secure, SameSite=Strict, path `/`, no `Domain`. The refresh token is never in a body (`SE1`, `TK7`) and is never stored: `refresh_tokens.token_hash` holds its SHA-256 (`NFR-SEC-07`). |
...
### Rotation, reuse and the question to ask first (`M-022`)

| Question | Answer today |
| :--- | :--- |
| A member says they were signed out of every device at once | Look up `revoked_reason` on their `auth_sessions` rows. `TOKEN_REUSE_DETECTED` means a spent generation was replayed and the whole family was revoked by design (`E1.2`); `USER_SIGNED_OUT` means they, or someone with their session, pressed the button. The reason is deliberately distinct so `ALRT-32` counts only the first — folding them together would bury the one alert that matters under every routine logout. |
| Is it theft, or the parallel-tab false positive? | The grace window makes the false positive **unlikely rather than impossible**, so still check. A replay within 10 seconds of the legitimate rotation returns the successor token and revokes nothing, and a cluster of near-simultaneous refreshes on one family is therefore already handled. A replay **minutes** later is what reaches the revocation path. If you are seeing revocations from near-simultaneous requests, the grace is not working — that is a defect, not an incident. |
| Genuine reuse — what is the containment action? | **None.** The family is already revoked; that is what the detection does. The work is identifying the member and notifying them (`E1.2`), then finding how the token left their browser. |
| A member cannot refresh, and no reuse was recorded | Three ordinary causes, in order of likelihood: the session was revoked (check `revoked_at`), the 30-day absolute expiry passed (`absolute_expires_at`), or a password reset revoked every session — which shares one code path with explicit revocation, by `AC-7`, so it looks identical here. All three are `EXPIRED` rather than an alarm, deliberately: a member refreshing after a password reset presents a spent generation and must not be reported as a thief. |
| Revocation "did not take" — the old token still works | It should not, and for at most as long as the access-token TTL. Revocation writes `auth:revoked-family:{fam}` to Redis and the guard checks it on every authenticated request (`AC-10`). The denylist **fails OPEN**: if Redis is down it denies nothing, because signing out the entire platform for a cache restart is the worse of the two failures. So the first thing to check is whether Redis was reachable at the moment of the complaint. |
| Can `used_at` be repaired by hand? | No, and not as `postgres` either. `trg_refresh_tokens__completion_is_write_once` refuses to clear or move it for every role — the grant says which columns, the trigger says once. A fixture that genuinely needs it uses `SET LOCAL session_replication_role = replica` inside one transaction; production has no such need, and wanting one is a signal to stop and ask why. |
