...
| — | **Admin console redesign** (off roadmap order, at the owner's request) | 🟡 `IN PROGRESS` — Phases 1–2 of 11 | — | Brand re-based on the `wine` ramp (`ADR-0036`, brand-600 vs danger-600 measured at 2.18:1) · `packages/ui` given its first components against the 1,946-line `Components.md` · ⌘K, `PageHeader`, `MetricCard` · **`turbo run lint` extended from five workspaces to eight**, which found the two currency formatters and a JSON-LD injection · new `gymmap/no-surface-currency-format` rule implementing `FolderStructure.md` §12 rule 14 · new `ci:tailwind-tokens` gate after 13 Tailwind classes were found emitting nothing |
| — | **Customer marketplace build-out** (off roadmap order, at the owner's request) | 🟡 `IN PROGRESS` — Phases 0–3 of 10 | — | Video hero (`SCR-WEB-001`) · site chrome with a native-`<dialog>` mobile drawer · photo result cards · the full home page — trust, listings, goals, how-it-works, cities, owners, close · **54/54** web unit · found four real defects: `next/image` `fill` positioning silently dropped by the CSP, `bg-surface/95` emitting no rule at all (a fully transparent sticky header), Next's route announcer unhidden by the same policy, and every Phosphor import on a name deprecated in 2.1 |
