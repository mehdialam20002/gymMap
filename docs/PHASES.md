...
| BLK-10 | **`§B3.2` defines no capability for the two platform-wide admin reads.** `apps/server/src/admin/permissions.ts` declares `admin.platform_overview.read` and `admin.gym_register.read`; neither is in the 42-capability matrix, so `M-023`'s `PermissionsGuard` refuses both as `UNKNOWN_PERMISSION` — which is deny-by-default working correctly and reveals that the two keys were invented by the module rather than derived from the PRD. The nearest matrix keys are wrong in a dangerous direction: `catalog.gym_profile.read` is the PUBLIC marketplace read held by `VISITOR`, so mapping the register onto it would expose every tenant's commercial terms to an unauthenticated visitor. The matrix is rank-2 PRD data and cannot gain a capability from an implementation decision. | 1 | Project owner + Security | 2026-08-08 | **OPEN, not blocking.** The two routes keep `PlatformRoleGuard` (`TD-034`) and are unchanged; `PermissionsGuard` is registered per route rather than globally, so it governs only routes whose permission IS a matrix capability. What is needed is either two new `§B3.2` rows under `MASTER_PRD` Part C §C10 change control, or a decision that `SCR-ADM-001` and `SCR-ADM-004` are covered by an existing capability. |
| BLK-11 | **`FR-RBAC-05`'s own endpoint declares a permission `§B3.2` does not define.** `API_Catalog.md` line 1077 freezes `GET /admin/users/:id/permissions` with permission `admin.user.read_permissions`; that key is not among the 42 capabilities, so `PermissionsGuard` would refuse the route as `UNKNOWN_PERMISSION`. This is `BLK-10`'s family and arguably its sharper form: the permission that gates the effective-permission INSPECTOR is itself absent from the matrix the inspector reports on. Mapping it onto a neighbour is not available either — the nearest keys are staff-management writes, and a support read must not imply a write. `API_Catalog.md` is rank-3 derived spec; the matrix is rank-2 PRD data and cannot gain a capability from it (`CLAUDE.md` §2). | 8 | Project owner + Security | 2026-08-09 | **OPEN, blocks the endpoint only.** The DOMAIN resolver is built and tested (`inspectPermission`, `annotatedEffectivePermissions` — 10 tests covering `AC-STAF-08.1`, `AC-STAF-08.3` and `Security.md` RB5), because none of it needs the permission key. What cannot be built is the route, its use case and the `AC-STAF-08.2` audit row, since all three begin with the capability that gates them. Resolving `BLK-10` and this together is one `§C10` change; resolving them separately invites two different answers to the same question. |
| BLK-12 | **The constitution and both shipped feature documents disagree about what a feature document is.** §21.3 `FD1` mandates five sections in a fixed order — Business Rules, Database, API, Flow, Future Improvements — and DoD item 23 requires all five; `DG3` requires a mermaid sequence diagram in every one. `docs/features/tenant-isolation.md` and `docs/features/session-lifecycle.md` both use a narrative spine instead (`## 1. What it is`, `## 2. How it is proved`, …) with ```text ASCII diagrams, and `M-015`'s own roadmap entry claims the file has "the five §21.3 sections" when it does not. CI job 22 checks only that a feature document EXISTS, which is why the drift survived two reviews. The divergence appears in none of `DECISION_LOG.md`, `TECH_DEBT.md` or `KNOWN_LIMITATIONS.md`, which `CLAUDE.md` §9.6 forbids. | 8 | Project owner + Technical Lead | 2026-08-09 | **OPEN, blocks `docs/features/rbac.md`.** `M-023`'s Files list names that path, so the destination is not in question — the SHAPE is. Writing it to §21.3 breaks consistency with the only two precedents; writing it in the house style violates the highest-authority document, and the precedence ladder says the constitution wins. Either a `DECISION_LOG.md` entry blessing the narrative style (an amendment to §21.3), or a `TECH_DEBT.md` row against the two existing files and §21.3 shape for the new one. Not a decision to take in a third file. |
| BLK-13 | **The hero image's alt text is specified twice, incompatibly.** `CustomerApp.md` line 447 says *"The hero image carries an empty `alt`"* — decorative. `apps/customer-web/test/hero.spec.ts` line 104 requires `alt={t('web.home.hero.photoAlt')}` and forbids `aria-hidden`, and the catalogue carries a real description. Both cannot hold. `ADR-0037` made layout, column order and the wireframes advisory; it did not touch alt-text policy, so this is not resolved by that decision. | 8 | Project owner + Accessibility | 2026-08-09 | **OPEN, not blocking.** The shipped behaviour is the descriptive alt, which is the safer default of the two — a described photograph is a minor verbosity cost, an undescribed one is content a screen-reader user cannot reach. Raised rather than reconciled because a spec conflict is not settled in code (`CLAUDE.md` §9.3), and because the spec may be right: a purely decorative gym photograph behind a headline is a legitimate empty-`alt` case. |
...
| BLK-15 | **`Schema.md` gives `kyc_checklists` two different grant classes, and they prescribe different DDL.** §1.3 (line 470) lists it among the GLOBAL reference tables *"written only by `admin/` through the audited reason-required path"* — the definition of **G-REF**, which §10.3 grants `SELECT, INSERT, UPDATE` to `app_rw`. §12.3 (line 2040) says *"Grants: **G-APPEND** on all three — versioned by validity window, superseded, never edited"* — `SELECT, INSERT`, and never `UPDATE`. §10.3's own illustrative G-APPEND grant list names `tax_profiles` and `subscription_tiers` and does **not** name `kyc_checklists`, so the file disagrees with itself in three places. This is not cosmetic: **CI-02** fails the build if a G-APPEND table carries `UPDATE` for `app_rw`, so the two readings cannot both be satisfied. The roadmap's M-029 entry says G-REF, but `docs/roadmap/` is *"a plan of work, never a source of requirements"* and does not settle it. | 8 | Project owner + Technical Lead | 2026-08-09 | **OPEN, not blocking today.** The M-029 migration grants **only the privilege both readings agree on — `SELECT`** — and defers the disputed write grant rather than picking a reading in code (`CLAUDE.md` §9.3). Nothing needs the difference yet: the seed runs as the migration role, and the `FR-ADMN-06` write path arrives at **M-116** (`Milestones_090-119.md` line 1716). Same posture as the committed `roles`, `permissions` and `role_permissions` grants. `kyc-checklists.int-spec.ts` asserts `app_rw=SELECT` and `app_platform_ro=SELECT` and nothing else, so the deferral is a tested state rather than an omission. **Must be settled before M-116 writes the admin path** — the honest question is whether publishing a checklist version may ever `UPDATE` a prior row (G-REF) or must always insert a successor and set `superseded_at` (G-APPEND, and what `ERD.md` §9.6's snapshot rule actually implies). |
| BLK-16 | **The KYC enclave is specified in four ways that cannot all be built, and two of its four separations have nowhere to live in configuration.** `Security.md` §8.3 defines the enclave as four separations — separate bucket, separate KMS CMK (`K-03`), separate storage credential (`K-09`), and no CDN ever. Four problems, and they are one decision rather than four: **(1) TTL.** `Security.md` `KY2` says the signed URL lives **300 seconds**; `Admin.md` §5.2 line 753 says **15 minutes**. Both are rank-3 `docs/` specifications, so neither outranks the other. **(2) Single-use.** `Admin.md` §5.2 and `CP3` both require the URL to be *"time-limited, **single-use**, audited"*. A presigned S3 GET is **not single-use** and the AWS SDK cannot make it so — delivering this needs a redirect endpoint that no specification describes. **(3) `K-03` and `K-09` have no configuration keys.** `app-config.schema.ts` declares one `S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY` pair shared between the media and KYC buckets, and no KMS key id at all — so two of §8.3's four separations are currently *unrepresentable*, not merely unimplemented. **(4) Runtime `block_public_access`.** `AC-7` asks for it to be asserted *"by both Terraform and a runtime check"*, and the only source for the runtime half is `docs/roadmap/`, which is "a plan of work, never a source of requirements". | 8 | Project owner + Security + Engineering / DevOps | 2026-08-09 | **OPEN, blocks `kyc-storage.adapter.ts` and `issue-kyc-access-url.use-case.ts`.** `A-18` approves the AWS SDK v3 S3 client, so nothing about the dependency blocks an adapter — what blocks it is that an adapter written today would pick one of the two TTLs, share the media credential, and skip single-use because the SDK cannot do it, then pass every test because the tests would be written against what it did. The enclave would exist on paper. `SprintPlanning` (quoted at `Milestones_000-029.md` §M-029) makes it a **rule** that the bucket, the key and the access log exist before the upload endpoint ships. So `ObjectStoragePort` is defined in `common/storage/` per `FolderStructure.md` §7.2, and the bound adapter answers **`UNAVAILABLE`** on every call — the `KL-099` pattern, with a per-operation refusal counter so the gap is a dashboard line rather than a silence. `isPrivate()` deliberately refuses rather than answering: `true` would be an unchecked all-clear on the exact control `AC-7` names, and the result union makes the third answer impossible to read past. Not split into four rows because they are one question — *what is the enclave* — and answering three of them leaves nothing buildable. |
| BLK-17 | **A KYC document access is specified as two different audit rows, and the writer cannot guarantee either.** `Schema.md` §4.3 and `Gym.md` imply `action = 'EXPORT'` on `entity_type = 'KYC_DOCUMENT'`; `Admin.md` §3.4 line 436 says the row is `action = 'ELEVATE'` with `elevation_scope = 'KYC_DOCUMENT_ACCESS'`. **Both are writable** — `elevation_scope` is `text NULL`, not an enum — so the wrong choice produces a silently wrong row rather than an error, and the audit explorer's `FR-ADMN-09` question *"who looked at this owner's passport, when, and why"* would be answered by a filter that misses half the rows. Compounding it: `AC-8` requires that *"no signed URL is issued unless the audit row committed first"*, and **neither existing path can promise that**. `AuditPrismaRepository.append()` swallows write failures by design (`BR-DAT-01` availability trade-off), so it cannot refuse; and `runElevated()`'s own documented guarantee — *"BEFORE the work. If this throws, the elevation does not happen"* — is already false in committed code for the same reason. `ELEVATION_SCOPES` is additionally a closed four-value list that would throw on `KYC_DOCUMENT_ACCESS`. | 8 | Project owner + Security | 2026-08-09 | **OPEN, blocks `issue-kyc-access-url.use-case.ts`.** Note this route is *not* caught by `BLK-14` — its permission key exists (`onboarding.kyc_document.read`, row 31 of `§B3.2`). One decision settles both halves: whether a KYC access goes through `runElevated()` — needing a fifth `ELEVATION_SCOPES` value and a strict, throwing write path — or through a direct `EXPORT` append, needing `AuditWritePort` to grow an `appendOrThrow` and thereby changing a deliberate availability trade-off. Either edits modules outside `onboarding/`, which is why it is the owner's rather than the milestone's. Same family as `BLK-10`, `BLK-11` and `BLK-14` and best decided with them. **Separately and already fixed, because it needed no ruling:** a client-supplied `X-Correlation-Id` that passed the log-injection sanitiser but was not a uuid failed the `::uuid` cast inside that same swallowing `catch`, so **any caller could suppress their own audit row with one header** — see `d9119ce`. |
| BLK-18 | **As the deployed audit credential, EVERY audit write fails — and the failure is swallowed, so `BR-DAT-01` is unsatisfiable in any environment that uses it.** `AuditPrismaService` connects as `gymmap_audit`, a member of `app_append` and nothing else, and is **deliberately not** extended with the tenant-context extension. Its own header says why: *"an audit row's `tenant_id` is frequently NULL (an elevation belongs to no tenant, and neither does a platform-admin login), and the extension refuses any operation with no tenant in scope."* So `app.tenant_id` is never set on that connection. But `rls_audit_log__tenant_isolation`'s WITH CHECK reads `(tenant_id = current_setting('app.tenant_id')::uuid) OR (tenant_id IS NULL)`, and the **one-argument** `current_setting` **raises** `42704` when the setting is absent. PostgreSQL does not promise to short-circuit `OR`, and here it does not — the left operand raises before the right one, which is `TRUE`, is ever reached. **Every insert fails, including exactly the `tenant_id IS NULL` rows that clause exists to permit**, and `AuditPrismaRepository.append()` catches and logs by design. Proved as `gymmap_audit` against the live database: the insert fails with `42704`, and the identical insert **succeeds** once `app.tenant_id` is set — isolating the cause to the missing setting rather than to the grant, the enums or a NOT NULL column. It is invisible locally because `AUDIT_DATABASE_URL` is unset and the documented fallback is `DATABASE_URL` → `postgres`, a **superuser, which bypasses RLS entirely**. `audit_log` held **zero rows** when this was found. | 8 | Project owner + Security + Schema Owner | 2026-08-09 | **OPEN, and it is the most consequential row in this table.** The obvious fix — `current_setting('app.tenant_id', true)` — was written, applied, and **reverted within the hour**, because `Constraints.md` §8.3 forbids it in one line: *"`missing_ok` is `false`. It is never written."* `migration-lint` enforces that rule and refused the migration; the rule is right, and its reasoning (a NULL predicate returns zero rows silently, the bug looks like missing data, somebody widens the policy to "fix" it, and *that* is the breach) is about the READ side, which is not what fails here. Two candidate fixes, both schema-owner calls: **(a)** a second, role-scoped policy permitting `INSERT` to `app_append` without the tenant predicate — the pattern `rls_audit_log__platform_read` already uses, and consistent with the service's stated position that *"isolation is instead the `app_append` grant: this pool can write one table and read nothing"*; or **(b)** the audit connection sets `app.tenant_id` explicitly before each write — rejected on first inspection, because a writer choosing the value it is then checked against is §8.4's tautology in a different costume. Pinned as a **canary** in `audit-correlation.int-spec.ts`: the test asserts the failure still happens and says, in its own failure message, what to do when it stops. |
| BLK-19 | **The five tenant branch routes need five permission keys, and two rank-3 documents give two irreconcilable answers — so four of the five routes cannot declare a permission at all.** `docs/engineering/API_Catalog.md` lines 940–944 freeze one key per route: `catalog.branch.list`, `.create`, `.read`, `.update`, `.deactivate`; `docs/apis/Gym.md` §12, `docs/ui/GymDashboard.md` and `docs/ui/Navigation.md` agree with it. `docs/engineering/Security.md` line 832 is the only document that maps all forty-two `§B3.2` rows to permission strings, and for row 20 (*"Add / remove branch"*) it gives exactly one: `catalog.branch.write`. The shipped `CAPABILITY_MATRIX` agrees with `Security.md` — `apps/server/src/iam/permissions.ts` lines 461–477 decompose the row into `catalog.branch.read` and `catalog.branch.write` and nothing else — and `PERMISSION_KEYS` is derived from that matrix, so it is the set `PermissionsGuard` validates against. **Only `catalog.branch.read` appears in both.** `MASTER_PRD.md` §B3.2 cannot break the tie: it defines no key strings at all, being a 42-row grid of human-readable labels against twelve role columns, and the string `catalog.branch` appears nowhere in it. Both documents are rank 3 under `CLAUDE.md` §2, and §9.3 forbids reconciling them in code. A second disagreement rides on the same row: `Gym.md` line 167 and `GymDashboard.md` line 988 grant the branch LIST to `GYM_MANAGER`, `RECEPTIONIST` and `TRAINER`, while `§B3.2` row 20 grants the capability to `GYM_OWNER` and `SUPER_ADMIN` only — so even picking `catalog.branch.read` silently drops the three branch-scoped roles `SCR-DASH-004` is built around. And a third: `Gym.md` line 170 maps `PATCH` to a *different* `§B3.2` capability (*"Edit gym profile"*) in order to give `GYM_MANAGER` a `▪`, so `catalog.branch.write` denies the manager while `catalog.gym_profile.update` lets an editor of a live listing edit branches. | 19 | Project owner + Security + Technical Lead | 2026-08-10 | **OPEN.** Exactly one of the five routes is buildable today: `GET /v1/tenant/branches/:id`, on `catalog.branch.read`, the single key both documents contain. The other four would declare a string `permits()` does not find in `PERMISSION_KEYS` and refuse **every** caller with `UNKNOWN_PERMISSION` — which is precisely what produced `BLK-10`, `BLK-11` and `BLK-14`, now for the fourth time. **CI cannot catch it:** `PG-3` validates the three-segment grammar and that the first segment is one of the 23 `§C1.3` modules, never registry membership, so an invented key passes `api-gates` and fails only when a request arrives, where it reads as a permissions bug rather than a missing PRD row. `Security.md` `RB3` specifies a registry-membership check; no such script exists in `packages/config/scripts/`, and adding one is the smallest thing that would have turned all four blockers into build failures instead of discoveries. The database layer of M-031 is built and green regardless — a table is not a route. |
| BLK-20 | **`state_code` is specified as two different alphabets by two rank-3 documents, and it decides which tax is charged.** `SeedStrategy.md` §3.6 seeds all 38 **GST numeric** codes (`27` Maharashtra, `29` Karnataka) and calls them *"the 2-digit prefix of every GSTIN"*; `Schema.md` derives `tenants.state_code` *"from `gstin` when present"*; `Gym.md` line 696 shows `"state_code": "27"` and line 1090 fixes GSTIN as *"2-digit **state code** + the 10-character **PAN**"*. Against that, `docs/apis/Marketplace.md` lines 662, 1694 and 1699 show `"state_code": "MH"`, `"KA"`, `"TG"` — **ISO alpha**. Both documents are rank 3. The consequence is not cosmetic: `state_code` is the **place of supply**, so it selects CGST+SGST versus IGST on every invoice a branch ever issues, and a mismatch is corrected only by a credit note per document. | 20 | Project owner + Finance + Schema Owner | 2026-08-10 | **OPEN, and I made it worse before recording it.** `M-031`'s `branches` migration shipped `ck_branches__state_code_shape CHECK (state_code ~ '^[A-Z0-9]{2}$')`, which admits `27` **and** `MH`. That is the conflict procedure's step 1 violated — *"do not implement both"* — dressed as a permissive constraint, and it is now the ONLY `state_code` constraint in the live database: `Constraints.md` specifies `ck_tenants__state_code_matches_gstin`, and `pg_constraint` shows it was never created, so `tenants.state_code` is unvalidated too. The check is deliberately left permissive rather than tightened, because tightening it to `^[0-9]{2}$` would be picking a side in code — but it is therefore **not a control**, and `catalog-tables.int-spec.ts` now pins that as a canary that says so in its own failure message. The numeric reading has the stronger structural claim (`ck_tenants__state_code_matches_gstin` is unbuildable against alpha, because a GSTIN's first two characters are digits), which is evidence for the owner and not a decision I may take. Whichever way it resolves, the other document is amended and both `state_code` columns get a real `CHECK` in the same change. |
| BLK-21 | **The reference tables' business KEYS are fixed and their DISPLAY columns are not, and the schema makes the display columns `NOT NULL` — so four of the five tables cannot be populated at all.** `SeedStrategy.md` §3.7 enumerates **59 amenity keys** in six display groups (Facilities 16, Equipment 16, Classes 12, Services 6, Accessibility 6, Hours 3) and **15 `gym_categories` keys**; §3.6 fixes **twelve city slugs** with their GST state code, `status = 'PLANNED'` and `timezone = 'Asia/Kolkata'`. Not one `amenities.name`, `amenities.icon`, `gym_categories.name`, `gym_categories.slug`, `cities.name` or `cities.centroid` is fixed anywhere — and `Schema.md` §12.1/§12.2 make every one of them `NOT NULL`, which the shipped migration faithfully reproduces. Writing the rows therefore means inventing 74 display names, 59 icons, 15 slugs, 74 sort orders and 12 coordinate pairs. `docs/backlog/Epic_04.md` line 349 already classifies exactly this as an unmade **client decision**: *"Amenity taxonomy content — the actual list of amenity terms and their display priority for the top-three card slots."* | 21 | Project owner + Product Designer + Client Sponsor | 2026-08-10 | **OPEN, and deliberately not worked around.** Three tempting substitutes were checked and each is worse than waiting. (1) **The shipped UI copy.** `apps/customer-web/.../fixtures/catalogue.ts` carries free-text strings — *"Free weights"*, *"Powerlifting platform"*, *"Valet parking"* — that map to **no** §3.7 key, and its own line 426 concedes it: *"When these become a controlled vocabulary … this export reads that vocabulary instead."* Importing it would create roughly a dozen invented amenities and zero valid category keys. (2) **The i18n catalogue.** `en.ts` holds 473 keys and its `// --- SCR-WEB-001 · categories ---` block is EMPTY; the only near-matches are six hero-chip labels. (3) **The API examples.** Every icon value in `Marketplace.md` and `Admin.md` — `rack`, `treadmill`, `locker`, `scooter`, `kettlebell`, `lotus` — is **not a Lucide name**, and `DesignSystem.md` line 1428 fixes Lucide as the library with `IC4` making glyph choice reviewable. Same for the four city centroids in `Marketplace.md` §10.1, which sit under a `// illustrative — not committed code` banner. **What IS built:** the five tables, their G-REF grants and their `§1.3` exemption entries (`1fa5591`), and the one row every column of which a specification fixes — `countries: IN`, per §3.1, with its id derived as `uuid_v5(NS_REFERENCE, 'countries:IN')` under `RD2`. **What is owed once the decision lands:** `prisma/reference/*.csv` as `RD4`'s source of truth (§1.4 fixes the filenames and row counts — `amenities.csv` 59, `gym-categories.csv` 15, `cities.csv` 120), `pnpm ref:generate` to emit the SQL, the `reference-data-drift` check of §2.7, and `RD3`'s sixteen-table DML allow-list in `migration-lint` — none of which exists yet, and all of which is mechanism rather than content. |
